...
//   const findUserByRegId = async (regId) => {
//     try {
//       console.log("Starting user search with regid:", regId)
      
//       const usersRef = collection(db, "users")
//       const regIdString = regId.toString().trim()
      
//       // Create query with correct field name (regid instead of regId)
//       const q = query(usersRef, where("regid", "==", regIdString))
      
//       console.log("Query created for regid:", regIdString)
...
//     try {
//       console.log("Attempting to assign book for student ID:", studentId)
...
function AssignBooks() {
  ...
  // Your existing useEffect and function implementations remain the same
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      if (searchTerm) {
        searchBooks(searchTerm)
